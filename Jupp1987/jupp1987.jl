...
df = CSV.read("Jupp1987/data/Jupp-etal-1987_dataset.csv", DataFrame; header=true)
...
regs = [Regularization(order=1, power=1.0, λ=0.001, diff_mode=FiniteDifferences(1e-4)), 
        Regularization(order=0, power=2.0, λ=0.1, diff_mode=FiniteDifferences(1e-4))]
...
plot_sphere(data, results, mean(df.Lat), mean(df.Lon), saveas="Jupp1987plots/plot_sphere.pdf", title="Double rotation")
plot_L(data, results, saveas="Jupp1987plots/plot_L.pdf", title="Double rotation")